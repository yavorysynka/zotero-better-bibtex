...
`Translator.<id>` where `<id>` is one of these four names. If you don't, you'll probably be OK, as in the
`BetterBib(La)TeX` context you will typically access `this`, which will be `null` in the `BetterCSL(JSON|YAML)` context,
and in the `BetterCSL(JSON|YAML)` context, you will typically access `csl` or `item`, which will be `null` in the
`BetterBib(La)TeX` context, which will cause an error to be raised and processing to be stopped. As postscript failures
...
<script type = 'text/javascript'>
          var redir = 'https://github.com/retorquere/zotero-better-bibtex/wiki/Scripting';
